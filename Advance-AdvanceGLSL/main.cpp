...
//    auto cube_texture = util::texture_from_file("container.jpg", "../resource/");
    ...
    unsigned int matrices_block;
    glGenBuffers(1,&matrices_block);
    glBindBuffer(GL_UNIFORM_BUFFER,matrices_block);
    glBufferData(GL_UNIFORM_BUFFER,sizeof(glm::mat4)*2,nullptr,GL_STATIC_DRAW);
    glBindBufferBase(GL_UNIFORM_BUFFER,2,matrices_block);
    glBufferSubData(GL_UNIFORM_BUFFER,sizeof(glm::mat4),sizeof(projection_matrix),&projection_matrix);
    ...
    while(!glfwWindowShouldClose(window)){
       util::process_input(window, eye_pos, camera_front, camera_up);
        ...
        glBindBuffer(GL_UNIFORM_BUFFER,matrices_block);
        auto buffer_point = glMapBuffer(GL_UNIFORM_BUFFER,GL_WRITE_ONLY);
        memcpy_s(buffer_point, sizeof(view_matrix), &view_matrix, sizeof(view_matrix));
